...
namespace AssetsModule {
	BoneAnimationKeys BoneAnimationKeys::readKeys(const aiNodeAnim* channel) {
		...
		for (auto i = 0u; i < channel->mNumPositionKeys; ++i) {
			...
			res.positions.emplace_back(SFE::Math::Vec3{ aiPosition.x, aiPosition.y, aiPosition.z }, channel->mPositionKeys[i].mTime);
		}
		...
		for (auto i = 0u; i < channel->mNumRotationKeys; ++i) {
			...
			res.rotations.emplace_back(SFE::Math::Quaternion<float>{ aiOrientation.w, aiOrientation.x, aiOrientation.y, aiOrientation.z }, channel->mRotationKeys[i].mTime);
		}
		...
		for (auto i = 0u; i < channel->mNumScalingKeys; ++i) {
			...
			res.scales.emplace_back(SFE::Math::Vec3{ scale.x, scale.y, scale.z }, channel->mScalingKeys[i].mTime);
		}
