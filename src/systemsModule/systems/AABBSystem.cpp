...
void Engine::SystemsModule::AABBSystem::update(const std::vector<ecss::SectorId>& entitiesToProcess) {
	ECSHandler::registry().forEach<ModelComponent, ComponentsModule::AABBComponent, TransformComponent>(entitiesToProcess, [this](auto entity, ModelComponent* modelComp, ComponentsModule::AABBComponent* aabbcomp, TransformComponent* transform) {
		...
		if (aabbcomp->aabbs.empty()) {
			aabbcomp->mtx.lock();
			aabbcomp->aabbs.resize(model.mMeshHandles.size());
			aabbcomp->mtx.unlock();
		}
		...
		for (size_t i = 0; i < aabbcomp->aabbs.size(); i++) {
			...
			const auto& meshHandle = model.mMeshHandles[i];
			const Math::Vec3 right	 { transformMatrix[0] * meshHandle.mBounds->extents.x};
			const Math::Vec3 up		 { transformMatrix[1] * meshHandle.mBounds->extents.y};
			const Math::Vec3 forward {-transformMatrix[2] * meshHandle.mBounds->extents.z};

			aabbcomp->mtx.lock();
			aabb.center = transformMatrix * Math::Vec4{meshHandle.mBounds->center, 1.f};
